 ...
 * @LastEditTime: 2025-04-10 03:24:01
...
DistanceProcess::DistanceProcess(const ros::NodeHandle& n) : nh_(n) {
  ...
  team_pose_ids_.resize(config_.num_robots_);
  ...
  for (size_t id = 0; id < config_.num_robots_; id++) {
    ros::param::get(config_.robot_names_[config_.my_id_] +
                        "/kimera_vio_ros/t_body_uwb" + std::to_string(id),
                    t_uwb_body_[id]);
  }

  std::string distance_topic = '/' + config_.robot_names_[config_.my_id_] + "/distance";
  distance_sub_ =
...
void DistanceProcess::requestGlobalPoses(const unsigned int robot_id) {
  ...
  for (size_t i = 0; i < request_global_pose_self.response.pose_ids.size(); i++) {
    ...
    team_global_poses_[config_.my_id_].emplace_back(timestamp, global_pose);

    // Add pose ID to team_pose_ids_ for my robot ID
    team_pose_ids_[config_.my_id_].push_back(pose_id);
  }
}
...
void DistanceProcess::distances2RelativePose(const std::vector<double>& distances,
  ...
  ROS_INFO_STREAM(
      "Optimized Relative Translation: " << relative_pose_opt.translation());
}
...
void DistanceProcess::runDistanceProcessLoop() {
  while (ros::ok()) {
    ...
    for (size_t id = 0; id < config_.num_robots_; id++) {
      ...
      auto& distance_queue = team_distance_queue_[id];
      ...
      while (!distance_queue.empty()) {
        ...
        if (team_global_poses_[config_.my_id_][team_latest_poses_idx_[id].first].first <
            process_meas.first) {
          auto it_i = std::lower_bound(
              team_global_poses_[config_.my_id_].begin() +
        ...
        ROS_INFO("Use the robor %ld 's %ld and robot %ld 's %ld",
                 config_.my_id_,
                 team_latest_poses_idx_[id].first,
                 id,
                 team_latest_poses_idx_[id].second);
        ...
        if (my_idx == team_latest_poses_idx_[id].first &&
          ...
          gtsam::Rot3 rotation_my_T =
              gtsam::Rot3::Quaternion(
                  team_global_poses_[config_.my_id_][my_idx].second.orientation.w,
                  team_global_poses_[config_.my_id_][my_idx].second.orientation.x,
                  team_global_poses_[config_.my_id_][my_idx].second.orientation.y,
                  team_global_poses_[config_.my_id_][my_idx].second.orientation.z)
                  .inverse();
          ...
          relative_translation_init =
              rotation_my_T *
              (gtsam::Point3(team_global_poses_[id][dst_idx].second.position.x,
                             team_global_poses_[id][dst_idx].second.position.y,
                             team_global_poses_[id][dst_idx].second.position.z) -
               gtsam::Point3(
                   team_global_poses_[config_.my_id_][my_idx].second.position.x,
                   team_global_poses_[config_.my_id_][my_idx].second.position.y,
                   team_global_poses_[config_.my_id_][my_idx].second.position.z));
          team_latest_relative_poses_[id] =
              gtsam::Pose3(relative_rotation_init, relative_translation_init);
          oR_i_next.emplace(gtsam::Rot3(rotation_my_T.inverse()));
        ...
        distances2RelativePose(process_meas.second,
                               ...
                               relative_translation_init,
                               relative_pose_opt);
        // 4th: Calculate the relative pose aligned to the pose estimation timestamp
        ...
        uint32_t use_idx_i = -1, use_idx_j = -1;
        int64_t delta_t_ri =
        ...
        if (use_idx_i < 0) {
          ...
          gtsam::Rot3 R_i_prev_T, R_j_prev_T;
          gtsam::Point3 t_i_next, t_i_prev, t_j_next, t_j_prev;
          t_i_next = gtsam::Point3(
              team_global_poses_[config_.my_id_][my_idx].second.position.x,
              team_global_poses_[config_.my_id_][my_idx].second.position.y,
              team_global_poses_[config_.my_id_][my_idx].second.position.z);
          t_i_prev = gtsam::Point3(
              team_global_poses_[config_.my_id_][my_idx - 1].second.position.x,
              team_global_poses_[config_.my_id_][my_idx - 1].second.position.y,
              team_global_poses_[config_.my_id_][my_idx - 1].second.position.z);
          R_i_prev_T =
              gtsam::Rot3::Quaternion(
                  team_global_poses_[config_.my_id_][my_idx - 1].second.orientation.w,
                  team_global_poses_[config_.my_id_][my_idx - 1].second.orientation.x,
                  team_global_poses_[config_.my_id_][my_idx - 1].second.orientation.y,
                  team_global_poses_[config_.my_id_][my_idx - 1].second.orientation.z)
                  .inverse();
          ...
          Delta_Ri_prev_r =
              gtsam::Rot3::Expmap(gtsam::Rot3::Logmap(R_i_prev_T * oR_i_next.value()) *
                                  (double)delta_t_ri / (double)Delta_t_i);
          Delta_ti_prev_r = R_i_prev_T * (t_i_next - t_i_prev) * (double)delta_t_ri /
                            (double)Delta_t_i;
        }
        ...
        if (use_idx_j < 0) {
          ...
          t_j_prev =
              gtsam::Point3(team_global_poses_[id][dst_idx - 1].second.position.x,
          ...
          if (!oR_j_next.has_value()) {
            oR_j_next.emplace(gtsam::Rot3::Quaternion(
                team_global_poses_[config_.my_id_][dst_idx].second.orientation.w,
                team_global_poses_[config_.my_id_][dst_idx].second.orientation.x,
                team_global_poses_[config_.my_id_][dst_idx].second.orientation.y,
                team_global_poses_[config_.my_id_][dst_idx].second.orientation.z));
          }
          Delta_Rj_r_prev =
              gtsam::Rot3::Expmap(-gtsam::Rot3::Logmap(R_j_prev_T * oR_j_next.value()) *
                                  (double)delta_t_rj / (double)Delta_t_j);
          Delta_tj_r_prev = Delta_Rj_r_prev * R_j_prev_T * (t_j_prev - t_j_next) *
                            (double)delta_t_rj / (double)Delta_t_j;
        }
        ...
        edge_dis.key_from =
            team_pose_ids_[config_.my_id_][my_idx - (use_idx_i < my_idx) ? 1 : 0];
